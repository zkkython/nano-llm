...
class LLaMa:
    ...
    def build(
        ...
        # 加载词表
        tokenizer = SentencePieceProcessor()
        tokenizer.Load(str(Path(checkpoint_dir) / tokenizer_path))
        ...
        model = Transformer(model_args=model_args).to(device)
        if load_model:
            del checkpoint["rope.freqs"]
            model.load_state_dict(checkpoint, strict=True)
            print(f"loaded state dict in {(time.time() - prev_time):.2f}s")
        return LLaMa(model=model, tokenizer=tokenizer, model_args=model_args)
...
if __name__ == "__main__":
    ...
    model = LLaMa.build(
        checkpoint_dir="/home/admin/llama/llam-7b/",
